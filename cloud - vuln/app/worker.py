...
def run_trivy_scan(
    ...
    """
    Execute Trivy scan with secure subprocess handling.
    ...
    cmd = [
        ...
        # Skip update check in production (use scheduled DB updates instead)
        "--skip-db-update",
    ...
    try:
        ...
        # shell=False is critical for security - prevents command injection
        process = subprocess.Popen(
            cmd,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            # Don't inherit environment variables that might affect behavior
            env={
                **os.environ,
        ...
        # Wait with timeout
        stdout, stderr = process.communicate(timeout=config.trivy_timeout)
    ...
    try:
        async with get_db_session() as session:
            ...
            if scan.status != ScanStatus.PENDING:
                logger.warning(
                    f"Scan {scan_id} is not pending (status={scan.status}), skipping"
                )
                return
        
        await process_single_scan(scan, config)
