...
class ScanService:
    ...
    async def submit_scan_request(
        ...
        """
        Submit a scan request - ALWAYS creates new scan for multiple scan support.
        
        MULTIPLE SCAN SUPPORT:
        ======================
        This method has been modified to support multiple scans of the same image.
        - ALWAYS creates a new scan with a unique ID
        - Each scan gets a unique ID regardless of image
        - No caching or in-progress checks - every request creates a new scan
        
        Args:
            image_name: Docker image name (e.g., "nginx", "python")
            image_tag: Image tag (default: "latest")
            registry: Container registry (default: "docker.io")
            force_rescan: Ignored - always creates new scan for backward compatibility
            background_tasks: FastAPI BackgroundTasks for async worker trigger
            triggered_by: User/system that initiated the request
            
        Returns:
            ScanResult with scan entity (newly_created=True always)
        ...
        logger.info(f"Processing scan request for: {full_image}")
        
        try:
            # =========================================================
            # MULTIPLE SCAN SUPPORT: Always create a new scan
            # No caching, no in-progress checks - every request is unique
            # =========================================================
            
            # Always create a new scan (allows multiple scans of same image)
            ...
            # Trigger background worker for the new scan
            if background_tasks:
    ...
    # async def _find_or_create_scan_atomic(
    #     self,
    #     image_name: str,
    #     image_tag: str,
    #     registry: str,
    ...
    # ) -> ScanResult:
    #     """
    #     DEPRECATED: No longer needed with multiple scan support.
    #     
    #     Previously atomically found an in-progress scan or created a new one.
    #     Now replaced with simpler logic that always creates new scans.
    #     """
    #     pass
    ...
    async def _create_scan(
        ...
        """
        # Generate unique idempotency key for tracking (no longer used for deduplication)
        ...
        # Create scan entity - each scan is unique regardless of image
        scan = VulnerabilityScan(
            image_name=image_name,
            image_tag=image_tag,
            registry=registry,
        ...
        await self.audit_repo.log_transition(
            ...
            message="Scan request received - multiple scan support enabled",
...
# =============================================================================
# STANDALONE HELPER FUNCTIONS - For API Layer
# =============================================================================
...
async def create_scan_request(
    ...
    """
    Standalone helper function to create a scan request.
    
    This wraps the ScanService class method for easier use from the API layer.
    ...
    Returns:
        VulnerabilityScan instance (either cached or newly created)
    """
    service = ScanService(session)
