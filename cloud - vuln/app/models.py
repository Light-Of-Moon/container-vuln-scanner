...
class ScanStatus(str, enum.Enum):
    ...
    """
    # Values must match PostgreSQL enum exactly (lowercase)
...
class ComplianceStatus(str, enum.Enum):
    ...
    # Aliases for backward compatibility with uppercase usage
    COMPLIANT = compliant
    NON_COMPLIANT = non_compliant
    PENDING_REVIEW = pending_review
...
class VulnerabilityScan(Base):
    ...
    # Idempotency key - used for deduplication (within time windows)
    # IMPORTANT: No unique constraint to allow multiple scans of same image
    idempotency_key: Mapped[str | None] = mapped_column(
        String(64),
        unique=False,  # Allow multiple scans of same image
        nullable=True,
        index=True,
        comment="Hash of (image_name + tag + timestamp_bucket) for optional deduplication"
    )
    ...
    status: Mapped[ScanStatus] = mapped_column(
        Enum(ScanStatus, name='scan_status', create_type=False, native_enum=True),
        nullable=False,
    ...
    compliance_status: Mapped[ComplianceStatus] = mapped_column(
        Enum(ComplianceStatus, name='compliance_status', create_type=False, native_enum=True),
        nullable=False,
    ...
    __table_args__ = (
        ...
        # FIXED: Use lowercase enum values to match ScanStatus enum
        Index(
        ...
        # NEW: Composite index for checking if scan is already in progress
        # Enables fast lookup: "Is there already a non-terminal scan for this image?"
        Index(
            "ix_scans_active_by_image",
            "image_name",
            "image_tag",
            "status",
            "created_at",
            postgresql_where=text(
                "status IN ('pending', 'pulling', 'scanning', 'parsing')"
            ),
        ),
...
class ScanAuditLog(Base):
    """Audit log for scan state transitions."""
    ...
    # IMPORTANT: Renamed from 'metadata' to 'audit_data' 
    # because 'metadata' is a reserved attribute in SQLAlchemy's Declarative API
